...
import type { NewsShowcaseSlice } from "@/prismicio-types";
	...
	return (
		<div className="mx-auto max-w-content text-center">
			...
			<h2 className="mt-3 font-display text-3xl font-bold sm:text-4xl md:text-5xl">
				{title_white ? <span className="text-white">{title_white} </span> : null}
				{title_gold ? <span className="text-pg-gold-bright">{title_gold}</span> : null}
			</h2>
		</div>
	);
}
...
function CardShell({
	...
	const className =
		"block overflow-hidden rounded-xl border border-pg-card-border bg-pg-card/90 text-left shadow-lg transition hover:border-pg-gold/40";
	if (href.startsWith("/")) {
...
function CardInner({
	...
	return (
		<>
			<div
				className="h-1 w-full rounded-full bg-gradient-to-r from-pg-blue to-pg-gold-bright"
				aria-hidden
			/>
			<div className="p-6">
				<div className="mb-4 flex items-center justify-between gap-2">
					{category ? (
						<span className="rounded-full bg-pg-gold px-3 py-1 text-xs font-bold text-pg-deep">
							{category}
						</span>
					) : (
						<span />
				...
				{headline ? (
					<h3 className="font-display text-lg font-bold text-white">{headline}</h3>
				) : null}
				{summary ? (
...
function formatArticleDate(d: ArticleDocument): string {
...
export default async function NewsShowcase({
	slice,
	context,
}: SliceComponentProps<NewsShowcaseSlice, SliceZoneContext>) {
	const { primary, items } = slice;
	const anchor = sectionIdFromKeyText(primary.section_id, "news");
	const lang = context.lang;
	/** Treat missing `feed` (pre–Slice-Machine-push repos) like `latest_articles`. */
